...
public class BookController {
    ...
    // ✅ Updated: Search by title, author, or genre
    @GetMapping("/search")
    public String searchBooks(@RequestParam String title, Model model) {
        List<Book> books = bookRepository.findByTitleContainingIgnoreCase(title);

        // If no match by title, try author
        if (books.isEmpty()) {
            books = bookRepository.findByAuthorContainingIgnoreCase(title);
        }

        // If still empty, try genre
        if (books.isEmpty()) {
            books = bookRepository.findByGenreIgnoreCase(title);
        }
